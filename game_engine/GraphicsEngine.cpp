...
GraphicsEngine::GraphicsEngine() : fpsAverage(0), fpsPrevious(0), fpsStart(0), fpsEnd(0), drawColor(toSDLColor(0, 0, 0, 255))
{
    ...
    // load font
    font = TTF_OpenFont("assets/ARCADE.TTF", 21);
...
void GraphicsEngine::drawText(const std::string& text, int x, int y, SDL_Color color)
{
    // for UI text
...
void GraphicsEngine::drawStar(int x, int y, int width, int height) {
    SDL_Rect renderQuad = { x, y, width, height };
    SDL_RenderCopy(renderer, starTexture, nullptr, &renderQuad);
}
